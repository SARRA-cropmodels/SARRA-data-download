...
def main():
    ...
    for x_ in config.dirs['step_evapotranspiration'].keys():
        config.dirs['step_evapotranspiration'][x_]["data_source"] = Path(config.bdir) / area  / config.dirs['step_evapotranspiration'][x_]["data_source"]
        config.dirs['step_evapotranspiration'][x_]["data_target"] = Path(config.bdir) / area  / config.dirs['step_evapotranspiration'][x_]["data_target"]

    cfg_dirs = config.dirs['step_evapotranspiration']

    newt_end = datetime.strptime(end, '%Y-%m') - relativedelta(days=2)
    end = newt_end.strftime("%Y-%m")

    parameters = config.Climat[model].keys()
    daterange = pd.date_range(datetime.strptime(start, '%Y-%m'),newt_end, freq='D') #?? s
    # Define config logger object
    config_logger = ConfigLogger(model, cfg_dirs)

    logging.info('#########################################################')
    logging.info('Start daily evapotranspiration of model data')
    ...
    for day in daterange:
        for parameter in parameters:
            ...
            data.read_files_2()
            data.combine_to_dataset_2()

            # If needed, calculate derived parameter from source parameters
            data.calc_derived_parameter_2()

            # Do daily aggregation
            # data.aggregate_to_daily_data_2()
...
def parse_config_CDSAPI_MIR_3(config_param, day, cfg_dirs):
    ...
    for i, my_var in enumerate(file_tag):
        ...
        cfg_timeslots[i] = {'timeslot': ts,
                            ...
                            'forecasthour': forecasthour,
                            'valid_date': valid_date,
                            'file_path': Path(path_dir) / path_file,
                            'data_type': data_type,

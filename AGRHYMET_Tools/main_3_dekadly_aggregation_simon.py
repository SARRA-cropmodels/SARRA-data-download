...
from AGRHYMET_Tools.lib.aggregation.data_synth import NetCDFData
...
def main():
    ...
    start = '20201101'  # Start date of processing period (20160401)
    end = '20201110'  # End date of processing period (20180331)
    start, end = str(sys.argv[1]), str(sys.argv[2])
    start_day = datetime.strptime(start, '%Y%m%d')
    end_day = datetime.strptime(end, '%Y%m%d')

    # Get model configuration
    cfg_dirs = config.dirs['step_dekadly_aggregation']
    parameters = config.DekadlyClimat[model].keys()
    ...
    logging.info('#########################################################')
    logging.info('Start dekadly parameter of model data')
    ...
    for parameter in parameters:
        logging.info('Process dekad / parameter: {} - {}/ {}'.format(start_day, end_day, parameter))

        # Ignore elements not beeing bias corrected

        param_name = config.DekadlyClimat[model][parameter]['ParamResult']['NcParamName']
        # if param_name not in config.excl_params_biascorrection:
        #     continue
        print(param_name)
        # Collect needed source files and timesteps
        cfg_parameter = config.DekadlyClimat[model][parameter]
        parse_func = parse_config_WebAPI_MIR

        if model is 'ERA5':
            # parse_func = parse_config_WebAPI_MIR_ERA5
            parse_func = parse_config_CDSAPI_MIR_4

        cfg_timeslots = parse_func(cfg_parameter, days, cfg_dirs)
        ...
        print(config_logger)
        print(cfg_timeslots)
        data = NetCDFData(cfg_parameter, cfg_timeslots, cfg_dirs, start_day, end_day, config_logger)
        data.read_files_3()
        data.combine_to_dataset_3()

        # If needed, calculate derived parameter from source parameters
        # data.calc_derived_parameter_3()

        # Do dekadly  aggregation

        data.aggregate_to_dekadly_data()
        # import pdb; pdb.set_trace()
        # Write final file to disk
        data.write_result_netcdf_2()
...
def parse_config(config_param, day, cfg_dirs):
    ...
    for i, ts in enumerate(timeslots):
        ...
        if data_type == 'FC':
            path_file = '{}_{}_{}_{}_{}.nc'.format(model, issue_date.strftime('%Y%m%d'), data_type,
                                                   issue_date.strftime('%H%M'), file_tag)
        elif data_type == 'AN':
            path_file = '{}_{}_{}_{}.nc'.format(model, issue_date.strftime('%Y%m%d'), data_type, file_tag)

        logging.info('    {} / {} \t -> {} +{}h \t -> {}'.format(day.strftime('%Y-%m-%d'), ts, issue_date, forecasthour,
                                                                 valid_date))
...
def parse_config_WebAPI_MIR_ERA5(config_param, day, cfg_dirs):
    ...
    for i, ts in enumerate(timeslots):
        ...
        if data_type == 'FC':
            path_file = '{}_{}_{}_{}_{}_mir_01gridNN.nc'.format(model, issue_date.strftime('%Y%m%d'), data_type,
                                                                issue_date.strftime('%H%M'), file_tag)
        elif data_type == 'AN':
            path_file = '{}_{}_AN_{}_mir_01gridNN.nc'.format(model, issue_date.strftime('%Y%m%d'), file_tag)

        logging.info(
            '    {} / {} \t -> {} +{}h \t -> {} \t -> {}'.format(day.strftime('%Y-%m-%d'), ts, issue_date, forecasthour,
                                                                 valid_date, path_file))
        cfg_timeslots[i] = {'timeslot': ts,
                            'issue_date': issue_date,
                            'forecasthour': forecasthour,
                            'valid_date': valid_date,
...
def parse_config_CDSAPI_MIR_3(config_param, day, cfg_dirs):
    """
    Parse configuration and create dict containing all needed source parameters, timesteps, files, etc
    """
    logging.info('  Parse timeslot config...')
    ...
    timeslots = config_param['Aggregation']['Timeslots']
    needed_params = config_param['ParamSource']['NcParamName']
    file_tag = config_param['GrbFileTag']
    data_type = config_param['GrbDataType']
    model = config_param['Model']
    ts = timeslots
    ...
    for i, my_var in enumerate(file_tag):
        issue_date, forecasthour, valid_date = parse_timestep_tag(ts, day, data_type)
        path_dir = '{}'.format(cfg_dirs[model]['data_source'])
        path_file = '{}_{}_{}.nc'.format(model, valid_date.strftime('%Y%m%d'), my_var)
        ma_variable = needed_params[i]
        #
        # for i, ts in enumerate(timeslots):
        #     issue_date, forecasthour, valid_date = parse_timestep_tag(ts, day, data_type)
        #     path_dir = '{}'.format(cfg_dirs[model]['data_source'])
        #     path_file = '{}_{}_{}.nc'.format(model, valid_date.strftime('%Y%m%d'), file_tag)
...
def parse_config_CDSAPI_MIR_4(config_param, days, cfg_dirs):
    """
    Parse configuration and create dict containing all needed source parameters, timesteps, files, etc
    """
    logging.info('  Parse timeslot config...')
    ...
    timeslots = config_param['Aggregation']['Timeslots']
    ma_variable = config_param['ParamSource']['NcParamName'][0]
    file_tag = config_param['GrbFileTag']
    data_type = config_param['GrbDataType']
    model = config_param['Model']
    ts = timeslots

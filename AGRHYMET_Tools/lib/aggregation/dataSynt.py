...
class NetCDFDataSynt():
    ...
    def combine_to_dataset(self):
        ...
        # # Replace lat/lon values, floating point error introduced somewhere at ECMWF, inaccurate already in downloaded gribs
        # import pdb; pdb.set_trace()
        # if self.cfg_parameter['Model'] == 'ERA5':
        #     merged.lon.data = np.arange(0, 360, 0.28125)
        #     merged.lat.data = np.arange(90, -90.001, -0.28125)
    ...
    def aggregate_to_daily_data_2(self):
        """
        Do temporal aggregation from 1-/3-hourly data to daily data according to predefined scheme
        """
        myAggr = Aggregator(self.cfg_parameter, self.day, self.config_logger)
    ...
    def write_result_netcdf(self):
        ...
        self.data_aggr.to_netcdf(outfile, engine='netcdf4', format='NETCDF4', encoding={parameter: {'zlib': True, '_FillValue': -9999}})
    ...
    def write_result_netcdf_2(self):
        ...
        outfile = out_filename + '.nc'
        self.data_der.to_netcdf(outfile, engine='netcdf4', format='NETCDF4', encoding={parameter: {'zlib': True, '_FillValue': -9999}})
